...
def user_sim(user_embed, threshold):
...
#             if i<j:
#                 edge = str(user_ids[i].item())+'-'+str(user_ids[j].item())
#                 weight = user_sim[i][j]
#                 if weight > threshold:
#                     pair[edge] = weight
...
#     user_src = [i+n_node for i in user_src]
#     user_tgt = [i+n_node for i in user_tgt]
    
#     weight = list(pair.values()) + list(pair.values())
#     usergnn_index = torch.tensor([user_src, user_tgt], dtype=torch.long).to(device)

    user_sim = torch.softmax(torch.mm(user_embed, user_embed.T), 1)
    edge_index = torch.stack(torch.where(user_sim > threshold), 0)
...
#     edge_count = torch.tensor(edge_count, dtype=torch.float)
    ...
    edge_index = torch.tensor([total_src, total_tgt], dtype=torch.long).to(device)
    # satgnn_index = torch.tensor([star, sat], dtype=torch.long).to(device)
    # stargnn_index = torch.tensor([sat, star], dtype=torch.long).to(device)
    # usergnn_index = torch.tensor([user_src, user_tgt], dtype=torch.long).to(device)
    edge_weight = torch.tensor([1]*len(src) + weight).to(device)
    # edge_weight = [i.to(device) for i in edge_weight]
    
    return (padded_embed, edge_weight, edge_index)
